...
def init_const():
    ...
    empty = get_empty_list(mas)  # переменная для поиска путых клеток
    random.shuffle(empty)  # метод для перемешивания элементов
...
def draw_game_over():
    ...
    img2048 = pygame.image.load('logo.png')
...
def game_loop():
    ...
    pygame.display.update()
    is_mas_move = False
    while is_zero_in_mas(mas) or can_move(mas):  # бесконечный цикл
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                save_game()
                pygame.quit()
